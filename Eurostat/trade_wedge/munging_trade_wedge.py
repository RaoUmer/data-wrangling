...
# Clean & join & write to csv. Deleting Footnoes.
frames = [gdp_c, gdp_n, imports_c, imports_n, demand_c, demand_n]
for df in frames:
    del df['Flag and Footnotes']

gdp_c = gdp_c.rename(columns={'Value': 'gdp_c'})
gdp_n = gdp_n.rename(columns={'Value': 'gdp_n'})
imports_c = imports_c.rename(columns={'Value': 'imports_c'})
imports_n = imports_n.rename(columns={'Value': 'imports_n'})
demand_c = demand_c.rename(columns={'Value': 'demand_c'})
demand_n = demand_n.rename(columns={'Value': 'demand_n'})

df = gdp_c.join(gdp_n, how='outer').join(imports_c, how='outer').join(
    imports_n, how='outer').join(demand_c, how='outer').join(
    demand_n, how='outer')

df = df.rename({'Euro area (17 countries)': 'EU17',
...
df.to_csv('trade_wedge_clean.csv')
